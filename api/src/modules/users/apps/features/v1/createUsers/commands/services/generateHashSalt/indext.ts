...
import { Container, ExceptionsWrapper, GuardWrapper, IServiceHandlerAsync, Result, ResultError, ResultFactory, sealed, Service, StatusCodes } from "@kishornaik/utils";
...
export interface ICreateUserGenerateHashAndSaltService extends IServiceHandlerAsync<ICreateUserGenerateHashAndSaltServiceParameters,ICreateUserGenerateHashAndSaltServiceResult> {}
...
export class CreateUserGenerateHashAndSaltService implements ICreateUserGenerateHashAndSaltService {
  ...
  public async handleAsync(params: ICreateUserGenerateHashAndSaltServiceParameters): Promise<Result<ICreateUserGenerateHashAndSaltServiceResult, ResultError>> {
    return await ExceptionsWrapper.tryCatchResultAsync(async ()=>{
      ...
      if(!hash || !salt)
        return ResultFactory.error(StatusCodes.INTERNAL_SERVER_ERROR,'Error while hashing password');
