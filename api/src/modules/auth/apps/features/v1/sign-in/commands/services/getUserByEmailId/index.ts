...
export interface IPublishGetUserByEmailIdIntegrationEventService
	extends IServiceHandlerAsync<
		IPublishGetUserByEmailIdIntegrationEventServiceParameters,
		IPublishGetUserByEmailIdIntegrationEventServiceResult
	> {}

@sealed
@Service()
export class PublishGetUserByEmailIdIntegrationEventService
	implements IPublishGetUserByEmailIdIntegrationEventService
{
	public async handleAsync(
		params: IPublishGetUserByEmailIdIntegrationEventServiceParameters
	): Promise<Result<IPublishGetUserByEmailIdIntegrationEventServiceResult, ResultError>> {
		return await ExceptionsWrapper.tryCatchResultAsync(async () => {
			...
			const replyMessageResult: ReplyMessageBullMq<JsonString> = await producer.sendAsync<
				JsonString,
				JsonString
			...
			const response: IPublishGetUserByEmailIdIntegrationEventServiceResult = JSON.parse(
				replyMessageResult.data
			) as IPublishGetUserByEmailIdIntegrationEventServiceResult;

			if (!response) return ResultFactory.error(StatusCodes.NOT_FOUND, 'User Not Found');
