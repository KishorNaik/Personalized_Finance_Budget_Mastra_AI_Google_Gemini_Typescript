import { Container, ExceptionsWrapper, GuardWrapper, IServiceHandlerAsync, IServiceHandlerVoidAsync, Result, ResultError, ResultFactory, sealed, Service, StatusCodes, VOID_RESULT, VoidResult } from "@kishornaik/utils";
...
export interface IValidateCredentialsService extends IServiceHandlerVoidAsync<IValidationCredentialsServiceParameters>{}
...
export class ValidateCredentialsService implements IValidateCredentialsService {
  ...
  public async handleAsync(params: IValidationCredentialsServiceParameters): Promise<Result<VoidResult, ResultError>> {
    return await ExceptionsWrapper.tryCatchResultAsync(async ()=>{

      const {credentials:{hash,salt},password}=params;
