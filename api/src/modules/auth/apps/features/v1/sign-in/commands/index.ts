...
import { SignInRequestDtoValidationService } from './services/validations';
import { IPublishGetUserByEmailIdIntegrationEventServiceResult, PublishGetUserByEmailIdIntegrationEventService } from './services/getUserByEmailId';
...
export class SignInCommandHandler  implements RequestHandler<SignInCommand, DataResponse<SignInResponseDto>> {

  private pipeline=new PipelineWorkflow(logger);
  private readonly _signInRequestDtoValidationService:SignInRequestDtoValidationService;
  ...
  public constructor(){
    this._signInRequestDtoValidationService=Container.get(SignInRequestDtoValidationService);
    this._publishGetUserByEmailIdIntegrationEventService=Container.get(PublishGetUserByEmailIdIntegrationEventService);
  ...
  public async handle(value: SignInCommand): Promise<DataResponse<SignInResponseDto>> {
    return await ExceptionsWrapper.tryCatchPipelineAsync(async ()=>{
      ...
      if (guardResult.isErr())
        return DataResponseFactory.error(guardResult.error.statusCode, guardResult.error.message);

      // Validation
      await this.pipeline.step(pipelineSteps.ValidationService, async () => {
        return await this._signInRequestDtoValidationService.handleAsync({
          dto: request,
          dtoClass: SignInRequestDto,
        });
      });

      // Get SignIn Response
      await this.pipeline.step(pipelineSteps.GetUserByEmailIdService, async () => {
        const result = await this._publishGetUserByEmailIdIntegrationEventService.handleAsync({
      ...
      await this.pipeline.step(pipelineSteps.ValidateCredentialsService, async () => {
        ...
        const getUserResult=this.pipeline.getResult<IPublishGetUserByEmailIdIntegrationEventServiceResult>(
      ...
      await this.pipeline.step(pipelineSteps.GenerateJwtToken,async ()=>{
        ...
        if(!jwtToken)
          return ResultFactory.error<string>(StatusCodes.UNAUTHORIZED,`user name and password do not match`);
      ...
      await this.pipeline.step(pipelineSteps.MapResponse,async ()=>{
        const jwtTokenResult=this.pipeline.getResult<string>(pipelineSteps.GenerateJwtToken);
        const getUserResult=this.pipeline.getResult<IPublishGetUserByEmailIdIntegrationEventServiceResult>(
          pipelineSteps.GetUserByEmailIdService
        );
