...
export class SignUpCommandHandler implements RequestHandler<SignUpCommand,DataResponse<SignUpResponseDto>>{
  ...
  public constructor(){
    this._publishCreateUserIntegrationEventService=Container.get(PublishCreateUserIntegrationEventService);
  }
  ...
  public async handle(value: SignUpCommand): Promise<DataResponse<SignUpResponseDto>> {
    const response= await ExceptionsWrapper.tryCatchPipelineAsync(async ()=>{
      ...
      await this.pipeline.step(pipelineSteps.publishUsrCreateIntegrationEvent,async ()=>{
        return await this._publishCreateUserIntegrationEventService.handleAsync(value.request);
      });

      // Get SignUp Response
      const response:SignUpResponseDto=this.pipeline.getResult<SignUpResponseDto>(pipelineSteps.publishUsrCreateIntegrationEvent);
